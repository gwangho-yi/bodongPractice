...
public class EditSpot extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Call<List<Spot>> call = spotInterface.SpotDetail(spotId,userEmailText);
...
//                onGetResult(response.body());
        ...
        mapSearchButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(EditSpot.this, AddSpotMap.class);
...
//            System.out.println("현재 이미지 리스트의 데이터들 : "+imgFiles);
        ...
        //태그 버튼 활성화 시키기
        tagArray = spotItem.getSpotTag();

        System.out.println("스팟 태그 리스트 : "+ tagArray);
        for (int i = 0; i < tagArray.size(); i++){
            if(tagArray.get(i).contains("트릭")){
                trickTagButton.setBackground(getResources().getDrawable(R.drawable.add_tag_check));
                trickTagButton.setTextColor(Color.parseColor("#ffffff"));
            }else if(tagArray.get(i).contains("댄싱")){
                dancingTagButton.setBackground(getResources().getDrawable(R.drawable.add_tag_check));
                dancingTagButton.setTextColor(Color.parseColor("#ffffff"));
            }else if(tagArray.get(i).contains("다운힐")){
                downHillTagButton.setBackground(getResources().getDrawable(R.drawable.add_tag_check));
                downHillTagButton.setTextColor(Color.parseColor("#ffffff"));
            }else if(tagArray.get(i).contains("슬라럼")){
                slalomTagButton.setBackground(getResources().getDrawable(R.drawable.add_tag_check));
                slalomTagButton.setTextColor(Color.parseColor("#ffffff"));
            }else if(tagArray.get(i).contains("프리라이딩")){
                freeRidingTagButton.setBackground(getResources().getDrawable(R.drawable.add_tag_check));
                freeRidingTagButton.setTextColor(Color.parseColor("#ffffff"));
            }
        }
        ...
        addSpotNameText.setText(spotItem.getSpotName());
        addSpotContentText.setText(spotItem.getSpotContent());
        locationName = spotItem.getSpotLocation();
...
//            System.out.println("태그리스트는 : "+tag);
        ...
        //삭제하는 이미지의 갯수가 1개 이상이면 실행을 하도록 한다.
        //삭제 이미지 리스트를 가지고 와서 서버에 보내준다. 그러면 서버에서는 삭제하는 이미지 리스트를 가지고 와서 삭제 이미지 카운트가 0 이상이라면 sql에서 이미지 테이블에서 이미지를 삭제시킨다.
        ...
        //스팟의 아이디
        RequestBody spot_id = RequestBody.create(MediaType.parse("text/plain"),
                spotId);
        //유저이메일
        ...
        Call<Spot> resultCall = userInterface.EditSpot(spot_id,images,userEmail,spotName,spotContent,number,tagNumber,locationText,map,latText,lngText,createTime,deleteImgMap,deleImgNumber);
...
//                Log.e("onSuccess", response.body().getMessage());
                ...
                Intent intent = new Intent(EditSpot.this, SpotDetail.class);
                intent.putExtra("spotId", response.body().getMessage());
                //기존에 있던 액티비티를 재사용하여 가지고 온다.
                //
